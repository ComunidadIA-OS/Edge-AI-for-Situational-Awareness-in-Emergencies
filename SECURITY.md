...
We aim to acknowledge reports within a few working days. Please include reproduction steps and the affected component (**vision** / **convergence** / **edge**).

## Secrets hygiene

The edge stack talks to external services (Open-Meteo, optional model hosting). Never commit credentials:

- `.env`, `.env.*`, and `*.local` are git-ignored — keep real secrets there, commit only `docker/.env.example`.
- If a credential is ever exposed, **rotate it immediately** and report via the channels above. A previously hardcoded third-party API key was found, removed, and rotated before this release.
...
- Third-party services (Open-Meteo) and their availability or terms.
